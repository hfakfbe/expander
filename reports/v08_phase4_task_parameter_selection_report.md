...
|---|---:|---:|---:|---:|---:|---:|---|
| copy | 6144 | 64/8 | 4x128 | 8 | 6250 | 5 | copy_token_accuracy |
| selective_copy | 8256 | 64/8 | 3x96 | 8 | 6250 | 5 | selective_copy_token_accuracy |
| induction_associative_recall | 1024 | 64/8 | 4x128 | 16 | 56250 | 5 | retrieval_exact_match |
| niah_kv_retrieval | 4160 | 64/8 | 3x96 | 8 | 6250 | 5 | retrieval_exact_match |
| ruler | 4160 | 64/8 | 3x96 | 8 | 7500 | 5 | retrieval_exact_match |
| lra_listops | 6016 | 64/8 | 4x128 | 8 | 60000 | 5 | listops_accuracy |
...
2. 训练预算：`resolved_train_budget_value` 统一按 `5 * ceil(resolved_train_examples / resolved_effective_batch_size)` 解析，表示每个 required method 计划训练 `5` 个 train-equivalent epochs，而不是一轮 sweep 或 smoke 式固定步数。
...
| resolved_train_budget_value | main 训练步数 | steps | 5 * ceil(train examples/effective batch) | 复现实验预算并证明不是单轮 sweep | smoke 使用 smoke.steps 并记录 |

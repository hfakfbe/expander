...
MAIN_TRAIN_EQUIVALENT_EPOCHS = 5
...
def _select_steps(train_rows: int, effective_batch: int) -> int:
    steps_per_epoch = max(1, math.ceil(int(train_rows) / max(int(effective_batch), 1)))
    return steps_per_epoch * MAIN_TRAIN_EQUIVALENT_EPOCHS
...
def build_task_record(task: str, audit_row: dict, output_dir: Path) -> dict:
    ...
    record = {
        ...
        "selection_reason": (
            ...
            f"main 训练预算解析为 {MAIN_TRAIN_EQUIVALENT_EPOCHS} * ceil(train_rows/effective_batch)={main_steps} steps，"
            f"每个 required method 计划看到 {main_steps * effective_batch} 个训练样本，"
            f"相当于 {MAIN_TRAIN_EQUIVALENT_EPOCHS} 个 train-equivalent epochs；"
        ...
        "resolved_train_equivalent_epochs": MAIN_TRAIN_EQUIVALENT_EPOCHS,
        "resolved_train_examples_seen_planned": main_steps * effective_batch,
        ...
        "main": {
            ...
            "train_examples_seen_planned": main_steps * effective_batch,
            "train_equivalent_epochs": MAIN_TRAIN_EQUIVALENT_EPOCHS,
...
def write_reports(rows: list[dict], output_dir: Path) -> None:
    ...
    for row in rows:
        table_rows.append(
            "| {task} | {resolved_padded_sequence_length} | {resolved_graph_block_size}/{resolved_graph_degree_or_budget} | {resolved_layers}x{resolved_d_model} | {resolved_effective_batch_size} | {resolved_train_budget_value} | {resolved_train_equivalent_epochs} | {primary_metric} |".format(**row)
        )
    report.write_text(
...
| task | padded T | B/d | 模型 | effective batch | main steps | train-equivalent epochs | primary metric |
|---|---:|---:|---:|---:|---:|---:|---|
...
2. 训练预算：`resolved_train_budget_value` 统一按 `{MAIN_TRAIN_EQUIVALENT_EPOCHS} * ceil(resolved_train_examples / resolved_effective_batch_size)` 解析，表示每个 required method 计划训练 `{MAIN_TRAIN_EQUIVALENT_EPOCHS}` 个 train-equivalent epochs，而不是一轮 sweep 或 smoke 式固定步数。
...
| resolved_train_examples | 训练集样本数 | 样本数 | Phase 1 audit | 计算每个 train-equivalent epoch 的步数 | 无 |
| resolved_train_budget_value | main 训练步数 | steps | {MAIN_TRAIN_EQUIVALENT_EPOCHS} * ceil(train examples/effective batch) | 复现实验预算并证明不是单轮 sweep | smoke 使用 smoke.steps 并记录 |
| resolved_train_equivalent_epochs | main 训练等效 epoch 数 | epochs | Phase 4 选择 | 证明主实验继续训练多轮 train-size 样本量 | smoke 写 not_applicable |
| resolved_train_examples_seen_planned | 计划看到的训练样本数 | 样本数 | steps * effective batch | 审计多 epoch 训练覆盖量 | smoke 按 smoke steps 记录 |
...
| resolved_train_equivalent_epochs | main 训练等效 epoch 数 | epochs | Phase 4 | 区分多轮训练和单轮 sweep | smoke 写 not_applicable |
| resolved_train_examples_seen_planned | 计划看到的训练样本数 | 样本数 | Phase 4 | 审计多 epoch 训练覆盖量 | smoke 按 smoke steps 记录 |
...
| training_curves_png_width | training_curves.png 宽度 | 像素 | run 后 PNG 审计 | 防止 1x1 空图误过 | 无 |
| training_curves_png_height | training_curves.png 高度 | 像素 | run 后 PNG 审计 | 防止 1x1 空图误过 | 无 |
| training_curves_png_satisfied | 曲线图审计是否通过 | true/false | result_field_audit.json | 确认曲线图非空且可读 | false 时 run 审计失败 |
...
| resolved_train_examples | 训练样本总数 | 样本数 | Phase 1 audit | 计算 train-equivalent epoch | 无 |
